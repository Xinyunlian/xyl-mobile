...
export default class Grid extends Component<GridProps, any> {
    ...
    render() {
        ...
        const renderItem = this.props.renderItem || ((dataItem: DataItem) => (
                ...
                >
                    {
                        isValidElement(dataItem.icon) ? dataItem.icon : (
        ...
        if (isCarousel && pageCount > 1) {
            for (let pageIndex = 0; pageIndex < pageCount; pageIndex++) {
                ...
                pagesArr.push(<div key={`pageitem-${pageIndex}`}
                                   className={`${prefixCls}-carousel-page`}>{pageRows}</div>);
            }
        }
        ...
        return (
            ...
            >
                {isCarousel && pageCount > 1 ?
                    <Carousel initialSlideWidth={this.clientWidth}>{pagesArr}</Carousel> : rowsArr}
            </div>
        );
    }
}
